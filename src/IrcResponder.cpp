...
private:
	...
	void sendWelcome() {
		...
		welcome += head + "001 " + bot->nick + " :Welcome to the Jenny IRC PassThru Server " + bot->nick + "\r\n";
...
void IrcResponder::connect() {
	...
	send("PASS " + pass);
	send("NICK " + nick);
	send("USER " + nick + " Jenny Bostrom " + config.host + " :http://multinode.ath.cx");
...
void IrcResponder::processLine(string &line) {
	string sender, command, params, pass;
	if (line[0] == ':') {
	...
	if (command == "JOIN") {
		...
		//	Send to console I suppose
                string::size_type ix = params.find("/QUOTE");
		if (ix!=string::npos){
			pass = params.substr(ix + 7);
                	cout << "\tSending " << pass << "\n";
			send (pass);
                }
