...
void HTTPListener::recv(string &s) {
	...
	client->write("Server: Jenny Web Server\r\n");
	...
	if (templateContents.length() == 0) {
		str = "<html>\n";
		str += "<head><title>I am Jenny!</title></head>\n";
		str += "<body>\n";
		str += "<center><h1>Welcome to multinode chat system</h1></center><br>\n";
                str += "<center>";
		str += botName;
		str += ": ";
		str += response;
		...
		str += "<form>\n";
		str += "<center>";
		str += "Response: <input type=text length=255 name=input> <input type=submit value=Say title=Send text=Send>";
                str += "</center>\n";
		str += "</form>\n";
		str += "</p>";
		str += "</body>";
